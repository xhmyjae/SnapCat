...
class PostRepository
{
    ...
    function getPostsbyFriend(User $connected_user): array
    {
        ...
        $statement = $this->databaseConnection->prepare('SELECT p.* FROM posts p JOIN friends f ON p.user_id = f.user_id1 OR p.user_id = f.user_id2 WHERE f.user_id1 = " '.$connected_user->id.' " OR f.user_id2 = " '.$connected_user->id.' " AND f.accepted = 1 LIMIT 5');
        $statement->execute();
