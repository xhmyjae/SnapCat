                    ...
                    <div class="post-footer">
                        <div class="post-reactions">
                            <form class="react-button" action="/add_reactions" method="POST">
                                <input type="hidden" name="post_id" value="<?= $post['id']; ?>">
                                <button type="submit" class="like-button form-btn" name="emoji" value="1">👍<span
                                            class="count <?php if ($reactionsCount->hasReactedWithEmoji(1, $post['id'], $connected_user->id)) echo 'reacted' ?>">
                                    <?php
                                    echo $reactionsCount->countReaction(1, $post['id']);
                                    ?>
                                </span></button>
                                <button type="submit" class="dislike-button form-btn" name="emoji" value="2">👎<span
                                            class="count <?php if ($reactionsCount->hasReactedWithEmoji(2, $post['id'], $connected_user->id)) echo 'reacted' ?>">
                                    <?php
                                    echo $reactionsCount->countReaction(2, $post['id']);
                                    ?>
                                </span>
                                </button>
                                <button type="submit" class="love-button form-btn" name="emoji" value="3">❤️<span
                                            class="count <?php if ($reactionsCount->hasReactedWithEmoji(3, $post['id'], $connected_user->id)) echo 'reacted' ?>">
                                    <?php
                                    echo $reactionsCount->countReaction(3, $post['id']);
                                    ?>
                                </span>
                                </button>
                                <button type="submit" class="sad-button form-btn" name="emoji" value="4">😭<span
                                            class="count <?php if ($reactionsCount->hasReactedWithEmoji(4, $post['id'], $connected_user->id)) echo 'reacted' ?>">
                                    <?php
                                    echo $reactionsCount->countReaction(4, $post['id']);
                                    ?>
                                </span>
                                </button>
                                <button type="submit" class="sad-button form-btn" name="emoji" value="5">😡<span
                                            class="count <?php if ($reactionsCount->hasReactedWithEmoji(5, $post['id'], $connected_user->id)) echo 'reacted' ?>">
                                    <?php
                        ...
                        <button class="comment-button <?= $post['id']; ?>"><i class="fa-regular fa-comment"></i>
                        </button>
